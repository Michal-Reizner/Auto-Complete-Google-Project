...
def get_score(whole_sentence, substring, index_of_mistake=-1, changed_letter=False):
    basic_score = len(substring) * 2

    if remove_punctuation(whole_sentence, [',', '.', ' ', '\n']).find(substring) != -1:
        return basic_score

    # score the string according to the mistake
    return switch_change(index_of_mistake, basic_score) if changed_letter else switch_add_remove(
        index_of_mistake, basic_score)
...
def add_to_dict(data, sentence: str, index_in_sentences_list):
    for i in range(minimal_input_len, len(sentence)):
...
def updade_matches_with_mistakes(data: dict, high_matches: list, input_, wrong_input, index_of_mistake,
    ...
    if data[wrong_input]:
        score = get_score(input_, wrong_input, index_of_mistake, changed_letter)
...
def get_matches_score(user_input: str, result: list) -> dict:
    scores_dict = {}
    for high_match_index in result:
        idx_in_sentence, idx_in_substring, max_sub_string = get_len_long_substr(sentences_list[high_match_index],
                                                                                user_input)
        if idx_in_substring == 0:
            if max_sub_string == len(user_input):
                score = get_score(sentences_list[high_match_index], user_input)
            else:
                score = get_score(sentences_list[high_match_index], user_input, max_sub_string, True)
        else:
            score = get_score(sentences_list[high_match_index], user_input, idx_in_substring, True)
...
def auto_complete(user_input: str) -> list:
    # if user want new search
    if user_input.find('#') != -1:
        return []

    auto_complete_list = []
    # result is a list with high matches for user input
    result = find_match(data, remove_punctuation(user_input, [',', '.', ' ', '\n']))
    scores_dict = get_matches_score(user_input, result)

    # sort matches according to their score
    result = list(reversed([k for k, v in sorted(scores_dict.items(), key=lambda item: item[1])]))

    # if there are no matches for user input
    if len(result) == 0:
        return [-1]

    else:
        # iterate over matches indexes list and create from each one AutoCompleteData object
        for sentence_index in result:
...
def print_high_matches(high_matches_list):
    if high_matches_list == [-1]:
        print("No Matches...")
        return

    if high_matches_list:
        for ind, auto_complete_data in enumerate(high_matches_list):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if len(user_input) >= minimal_input_len:
            high_matches_list = auto_complete(user_input)
            print_high_matches(high_matches_list)

        if user_input.find('#') != -1 or high_matches_list == [-1]:
